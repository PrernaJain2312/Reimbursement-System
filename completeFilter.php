...
if (isset($_POST['billUsername']) && isset($_POST['billType']) ) {
    ...
    $q2 = $_POST['billType']; 
    $allData = "SELECT * FROM bills WHERE (username='".$q1."' && bill_type='".$q2."' && status='Approved') ORDER BY bill_startDate";
    ...
    while($row1 = mysqli_fetch_array($allDataResult)){
        ...
        ?>
        <td><a href="complete.php?id=<?php echo $row1["bill_id"]; ?>" class="btn btn-primary btn-block btn-sm">Complete</a></td>   
        <?php
        echo "</tr>";
    }
    ...
    $allData = "SELECT * FROM bills WHERE (username='".$q."' && status='Approved') ORDER BY bill_startDate";
    $allDataResult = mysqli_query($dbc, $allData);
    while($row1 = mysqli_fetch_array($allDataResult)){
        $imageURL = "uploads/".$row1["bill_proof"];
        echo "<tr>";
    ...
    $allData = "SELECT * FROM bills WHERE (bill_type='".$q."' && status='Approved') ORDER BY bill_startDate";
    $allDataResult = mysqli_query($dbc, $allData);
    while($row1 = mysqli_fetch_array($allDataResult)){
        $imageURL = "uploads/".$row1["bill_proof"];
        echo "<tr>";
    ...
    $allData = "SELECT * FROM bills WHERE status='Approved' ORDER BY bill_startDate";
    $allDataResult = mysqli_query($dbc, $allData);
    while($row1 = mysqli_fetch_array($allDataResult)){
        $imageURL = "uploads/".$row1["bill_proof"];
        echo "<tr>";
